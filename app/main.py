...
class ListScreen(Screen):
    ...
    def cam_capture_video(
        ...
        cv2.imwrite(
            joined + "RotatedBody.jpg", resultBody, [cv2.IMWRITE_JPEG_QUALITY, 90]
        )
        cv2.imwrite(
            joined + "RotatedSolder.jpg", resultSolder, [cv2.IMWRITE_JPEG_QUALITY, 90]
        )
        ...
        cv2.imwrite(
            joined + "MaskSolder.png", maskSolder, [cv2.IMWRITE_PNG_COMPRESSION, 9]
        )
        cv2.imwrite(joined + "Raw.jpg", frame2, [cv2.IMWRITE_JPEG_QUALITY, 90])
        if self.capture_video_marker:
            self.project_data["Marker"] = imageprocessing.convert_image_to_base64(
                cropped
            )
    ...
    def cam_teachin_part(
        ...
        cv2.putText(
            rotated,
            self.overlay_teachin_designator + "  " + self.overlay_teachin_footprint,
            (10, 35),
            ...
            if (
                ...
                if partref != -1:
                    part = inspection.get_part_definition(
                        self.project_data["PartsDefinition"]["PartsDefinition"], partref
                    )
    ...
    def import_file(self):
        ...
        self._popup = Popup(title="Import file", content=content, size_hint=(0.9, 0.9))
        self._popup.open()
        self.project_data["CADMode"] = "None"
    ...
    def select_side(self):
        ...
        self._popup = Popup(
            title="Select Soldering Side", content=content, size_hint=(0.5, 0.6)
        )
        self._popup.open()
        self.project_data["CADMode"] = "None"
    ...
    def select_teachin_save(self):
        index = inspection.helper_get_index_by_designator(
            self.project_data["InspectionPath"], self.overlay_teachin_designator
        )
    ...
    def set_num_panel(self):
        ...
        self._popup = Popup(
            title="Select panel num", content=content, size_hint=(0.5, 0.4)
        )
        self._popup.open()
        self.project_data["CADMode"] = "None"
    ...
    def set_part_overlay(self, partindex):
        ...
        if partref != -1:
            ...
            if self.project_data["InspectionSide"] == "Top":
                self.overlay_teachin_rotation = (
                    inspectpart["Rotation"] - part["Rotation"]
                )
            else:
                self.overlay_teachin_rotation = (
                    part["Rotation"] - inspectpart["Rotation"]
                )
        else:
    ...
    def set_printer(self):
        ...
        self._popup = Popup(
            title="Select Printer port", content=content, size_hint=(0.5, 0.4)
        )
        self._popup.open()
        self.project_data["CADMode"] = "None"
    ...
    def set_camera(self):
        ...
        self._popup = Popup(
            title="Select Camera port", content=content, size_hint=(0.5, 0.4)
        )
        self._popup.open()
        self.project_data["CADMode"] = "None"
