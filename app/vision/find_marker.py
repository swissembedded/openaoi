...
def is_point_in_body_area(pixscale, bodyShape, bodySize, centerX, centerY, X, Y):
    ...
    if bodyShape == "Rectangular":
        ...
        if (
            np.square(deltaX) / np.square(cxhalf)
            + np.square(deltaY) / np.square(cyhalf)
            <= 1
        ):
            return True
    return False
...
def find_marker(
    ...
    template_loc = image_processing.helper_find_template(
        image_gray, template_gray, template_threshold
    )
    ...
    keypoints = image_processing.helper_find_blob(
        image_gray,
        pixscale,
        bodyShape,
        bodySize,
        ...
        for pt in zip(*template_loc[::-1]):
            if is_point_in_body_area(
                ...
                marker = {
                    ...
                    "Diameter": k.size,
                    "Distance": np.sqrt(
                        np.square(k.pt[0] - hintPos[0])
                        + np.square(k.pt[1] - hintPos[1])
                    ),
                }
